...
Making sure that `allen_core` and `allen_sce` are comparable.

```{r comparingData}
allen_sce<-ReprocessedAllenData()
allen_sce

allen_core
```

What are the extra samples in `allen_core`?
...
```{r zifa}
# NB: The only reason that digest was not working was that I was trying to run this without first loading the digest library
