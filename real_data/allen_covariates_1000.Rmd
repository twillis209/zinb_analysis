...
# Data without spike-ins, core only 
allen_core <- allen[grep("^ERCC-", rownames(allen), invert = TRUE), which(colData(allen)$Core.Type=="Core" & !(colData(allen)$Primary.Type %in% c("Pvalb Tacr3", "Sst Myh8")))]

filter <- rowSums(assay(allen_core)>10)>=10
```
We should end up with 285 cells as per the paper but we have 283 here.
...
```{r zinb}
# Removing ncores = ncores argument as unused
...
save(zinb, zinb_bio, zifa_fq, zifa_tmm, zifa_tc, zifa_raw, pc_deseq, pc_tmm, pc_fq, pc_tc, pc_raw, file="allen_covariates_1000.rda")
```
