...
```{r zinb}
# Removing ncores argument
zinb <- zinbFit(core, K = 2, epsilon = 1e3, 
...
pdf('allen_plots/gof_mu.pdf', width=10, height=5)
par(mfrow=c(1,2))
xlim =  c(5,9.5)
ylim = c(-1, 3)
...
```{r}
pdf('allen_plots/gof_pi.pdf', width=10, height=5)
par(mfrow = c(1, 2))
...
```{r}
pdf('allen_plots/gof_disp.pdf', width=10, height=5)
par(mfrow = c(1,2))
xpred = seq(0, 1, .05)
