...
counts <- read.table("glioblastoma_raw_rnaseq_SCandbulk_counts_withannots.txt", header=TRUE, stringsAsFactors = FALSE)
...
metadata <- read.table("SraRunTable.txt", sep=',', stringsAsFactors = FALSE, header=TRUE)
...
zinb <- fitZinbAndCache(core, K = 2, epsilon = 1e3, commonDispersion = FALSE)
```
...
```{r meanCount25}
pdf('patel_plots/gof_mu.pdf', width = 10)
par(mfrow=c(1,2))
xlim =  c(3.5, 10)
ylim = c(-1, 3)
...
```{r}
pdf('patel_plots/gof_pi.pdf', width = 10)
par(mfrow = c(1, 2))
...
```{r}
pdf('patel_plots/gof_disp.pdf', width = 10)
par(mfrow = c(1,2))
xpred = seq(0, 1, .05)
